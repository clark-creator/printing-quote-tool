      ...
      <div className="space-y-3 text-sm text-gray-700">
        ...
        <div>
          ...
          <div className="ml-3 space-y-2 text-xs">
            {/* Ink breakdown per line item */}
            {lineItemPricing.map((item) => (
        ...
        {/* Device Costs */}
        {lineItemTotals.deviceCostFloor > 0 && (
          <div>
            <p className="font-semibold text-red-800 mb-1">Device Supply (raw cost):</p>
            <div className="ml-3 space-y-1 text-xs">
              {lineItemPricing.map((item) => (
                item.deviceCostFloor > 0 && (
                  <div key={item.id} className="flex justify-between">
                    <span>
                      {item.deviceName}: ${item.deviceCost.toFixed(2)} × {item.quantity.toLocaleString()}
                    </span>
